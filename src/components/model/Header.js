...
import { ReactComponent as CrossIcon} from "../image/CrossIcon.svg";
...
const Header = ({ queryString = "", onClose = () => {} }) => {
  return (
    <div className="py-20 pl-18 pr-44" style={{backgroundColor: "#5c9ff0"}}>
        <HeaderTitle queryString={queryString} onClose={onClose}></HeaderTitle>
        <HeaderSubtitle queryString={queryString}></HeaderSubtitle>
...
const HeaderTitle = ({ queryString, onClose }) => {
    return (
      <>
        <div className="flex text-lg justify-between pb-15">
          {queryString ? QUERY_TITLE : DEFAULT_TITLE}
          <span
            className="flex bg-sky-700 p-1.5 my-0.5 rounded-md"
            onClick={onClose}
          >
            <CrossIcon
